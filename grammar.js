...
(() => {
	...
	const daysOfWeek = [
		...
		'friday',
		'saturday',
	].reduce((acc, m, i) => [...acc, [m, i], [m.slice(0, 3), i]], []);
	...
	return {
		lex: {
			...
			rules: [
				...
				// numbers
				[b(any(ordinals.map(([p]) => p))), `return 'ORDINAL'`],
				...
				// days of the week
				[b(any(daysOfWeek.map(([p]) => p))), `return 'DAY_OF_WEEK'`],

				// months
				[b(any(months.map(([p]) => p))), `return 'MONTH_OF_YEAR'`],
			],
		...
		bnf: {
			E: [
				...
				['abs_time', `$$ = $1`],
				['abs_date', `$$ = $1`],
				['abs_date abs_time', `$$ = [...$1, ...$2]`],
				['abs_date AT abs_time', `$$ = [...$1, ...$3]`],
				['rel_date', `$$ = $1`],
				['rel_date abs_time', `$$ = [...$1, ...$2]`],
				['rel_date AT abs_time', `$$ = [...$1, ...$3]`],
				['rel_time', `$$ = $1`],
				[
					// 	'rel_time abs_time',
					// 	`$$ = [...$1, ...$2]`
			...
			int_4: [['INT_4', `$$ = parseInt($1)`]],
			int_1_2: [['INT_1_2', `$$ = parseInt($1)`]],
			int: [['INT', `$$ = parseInt($1)`]],
			...
			rel_time: [
				[
					'IN duration',
					`$$ = $2.map(([u, x]) => ['add' + u[0].toUpperCase() + u.slice(1), x])`,
				],
				[
					'duration AGO',
					`$$ = $1.map(([u, x]) => ['sub' + u[0].toUpperCase() + u.slice(1), x])`,
				],
			],
			day_of_week: [
				['DAY_OF_WEEK', `$$ = ${json(idx(daysOfWeek))}[$1.toLowerCase()]`],
			],
			month_of_year: [['MONTH_OF_YEAR', `$$ = ${json(idx(months))}[$1.toLowerCase()]`]],
			rel_date: [
				// +- 1 Day
				// Implies same time
				['TODAY', `$$ = []`],
				...
				// Absolute day of week (e.g. monday, this fri, last tuesday)
				// Implies same time
				['day_of_week', `$$ = [['setDay', $1]]`],
				['THIS day_of_week', `$$ = [['setDay', $2]]`],
				[
					'NEXT day_of_week',
					`$$ = [['addWeeks', 1], ['setDay', $2]]`,
				],
				...
				['month_of_year', `$$ = [['setMonth', $1]]`],
				['THIS month_of_year', `$$ = [['setMonth', $2]]`],
				[
					'NEXT month_of_year',
					`$$ = [['addYears', 1], ['setMonth', $2]]`,
				],
				[
					'LAST month_of_year',
					`$$ = [['subYears', 1], ['setMonth', $2]]`,
				],
			],

			ordinal: [['ORDINAL', `$$ = ${json(idx(ordinals))}[$1]`]],
			abs_day_month: [
				['int_1_2 month_of_year', `$$ = [['setDate', $1], ['setMonth', $2]]`],
				['month_of_year int_1_2', `$$ = [['setDate', $2], ['setMonth', $1]]`],
				['ordinal OF month_of_year', `$$ = [['setDate', $1], ['setMonth', $3]]`],
				['ordinal month_of_year', `$$ = [['setDate', $1], ['setMonth', $2]]`],
				['month_of_year ordinal', `$$ = [['setDate', $2], ['setMonth', $1]]`],
			],

			year: [['int_4', '$$ = $1']],
			abs_date: [
				['abs_day_month', `$$ = $1`],
				['abs_day_month year', `$$ = [...$1, ['setYear', $2]]`],
			],
		},
	};
})()
